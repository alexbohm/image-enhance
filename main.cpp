...
void enhaceSquare(int w, int h, int channel, int sSize = 4){
	//temp threshold
	int th;
	//maximum thresholds
	int th1 = 0;
			...
			for(int x = w; x < w + 3; x++){
				...
				for(int uh = 0; uh < 3; uh++){
					for(int uw = 0; uw < 3; uw++){
						mod( (int)(x * ((double)sSize / 3.0)) + uw, (int)(y * ((double)sSize / 3.0)) + uh, channel) = c;
					}
				}
			}
		}
	...
	if(th1x == th2x){
		...
		for(int i = 0; i < 3; i++){
			for(int j = 0; j < 3; j++){
				if(i <= th1x - w){
					mod((int)(w * ((double)sSize / 3.0)) + i, (int)(h * ((double)sSize / 3.0)) + j, channel) = color1;
				} else if(i > th1x - w){
					mod((int)(w * ((double)sSize / 3.0)) + i, (int)(h * ((double)sSize / 3.0)) + j, channel) = color2;
				}
			}
		}
	} else {
...
int main(int argc, char const *argv[])
{
	...
	mod = CImg<unsigned char>(orig.width() * 6.0 / 3.0 + 1, orig.height() * 6.0 / 3.0 + 1, orig.depth(), orig.spectrum(), 0);
